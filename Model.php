...
abstract class Model
{
            ...
            foreach($rules as $rule) {
                ...
                if($ruleName === self::RULE_UNIQUE ) {
                    ...
                    $statement = Application::$app->db->pdo->prepare(
                        "SELECT id 
                               FROM $tableName
                               WHERE $uniqueAttr = :uniqueAttrValue 
                                ");
                    $statement->bindParam(":uniqueAttrValue" , $value);
                    $statement->execute();
    ...
    private function addErrorForRule(string $attribute, string $rule , $params = [])
    {
        ...
        foreach($params as $key=>$value) {
            if(is_int($value)) {
                ...
                $message = str_replace("{{$key}}", $this->toProperCase($value), $message);
            }
        }
    ...
    public function toProperCase($word) {
        return (strtoupper($word[0]) . strtolower(substr($word,1))) ?? $word;
    }
}